from tokenizerOHE_LSTM6Layers import letter_to_index, letter_to_tensor, sequence_to_tensor, loadData, codon_to_tensor, \
    tensor_to_codon, index_to_tensor, updateANDsave_heatmap, codon_to_tensor_12bits, tensor3x4_to_codon
from tokenizerOHE_LSTM6Layers import nucleotideLetters, aminoAcidLetters, nucleotideLetterList, \
...
filePATH = r"/content/deep-learning-project-cracking-the-genetic-code/DATA/"
...
root_dir = r"/content/deep-learning-project-cracking-the-genetic-code/DATA/"
...
hidden_features_layer2 = 128  
hidden_features_layer3 = 128
hidden_features_layer4 = 64 
hidden_features_layer5 = 64  
hidden_features_layer6 = 32  
...
'''
num_epochs = 3 # number of iterations in the training loop
...
class LSTM(nn.Module):
    def __init__(self, 
                input_features, 
                hidden_features_layer1, 
                hidden_features_layer2, 
                hidden_features_layer3, 
                hidden_features_layer4, 
                hidden_features_layer5,
                hidden_features_layer6,
                num_layers, 
                num_of_classes):
        super(LSTM, self).__init__()

        self.hidden_features_layer1 = hidden_features_layer1
        self.hidden_features_layer2 = hidden_features_layer2
        self.hidden_features_layer3 = hidden_features_layer3
        self.hidden_features_layer4 = hidden_features_layer4
        self.hidden_features_layer5 = hidden_features_layer5
        self.hidden_features_layer6 = hidden_features_layer6
        ...
        self.lstm1 = nn.LSTM(input_features, hidden_features_layer1, num_layers, batch_first=True)
        self.lstm2 = nn.LSTM(input_features, hidden_features_layer2, hidden_features_layer1, batch_first=True)
        self.lstm3 = nn.LSTM(input_features, hidden_features_layer3, hidden_features_layer2, batch_first=True)
        self.lstm4 = nn.LSTM(input_features, hidden_features_layer4, hidden_features_layer3, batch_first=True)
        self.lstm5 = nn.LSTM(input_features, hidden_features_layer5, hidden_features_layer4, batch_first=True)
        self.lstm6 = nn.LSTM(input_features, hidden_features_layer6, hidden_features_layer5, batch_first=True)
        # 1, 2 or 3 fully connected layers:
        self.fc1 = nn.Linear(hidden_features_layer6, num_of_classes, bias=False)
    ...
    def forward(self, input_tensor):
        ...
        h1 = torch.empty(self.hidden_features_layer1, input_tensor.size(0), self.hidden_features_layer2)
        h1 = nn.init.orthogonal_(h1, gain=1).to(device)
        c1 = torch.empty(self.hidden_features_layer1, input_tensor.size(0), self.hidden_features_layer2)
        c1 = nn.init.orthogonal_(c1, gain=1).to(device)

        h2 = torch.empty(self.hidden_features_layer2, input_tensor.size(0), self.hidden_features_layer3)
        h2 = nn.init.orthogonal_(h2, gain=1).to(device)
        c2 = torch.empty(self.hidden_features_layer2, input_tensor.size(0), self.hidden_features_layer3)
        c2 = nn.init.orthogonal_(c2, gain=1).to(device)

        h3 = torch.empty(self.hidden_features_layer3, input_tensor.size(0), self.hidden_features_layer4)
        h3 = nn.init.orthogonal_(h3, gain=1).to(device)
        c3 = torch.empty(self.hidden_features_layer3, input_tensor.size(0), self.hidden_features_layer4)
        c3 = nn.init.orthogonal_(c3, gain=1).to(device)

        h4 = torch.empty(self.hidden_features_layer4, input_tensor.size(0), self.hidden_features_layer5)
        h4 = nn.init.orthogonal_(h4, gain=1).to(device)
        c4 = torch.empty(self.hidden_features_layer4, input_tensor.size(0), self.hidden_features_layer5)
        c4 = nn.init.orthogonal_(c4, gain=1).to(device)

        h5 = torch.empty(self.hidden_features_layer5, input_tensor.size(0), self.hidden_features_layer6)
        h5 = nn.init.orthogonal_(h5, gain=1).to(device)
        c5 = torch.empty(self.hidden_features_layer5, input_tensor.size(0), self.hidden_features_layer6)
        c5 = nn.init.orthogonal_(c5, gain=1).to(device)
        ...
        output, hupdated = self.lstm1(input_tensor, (h0, c0))
        output, hupdated = self.lstm2(input_tensor, (h1, c1))
        output, hupdated = self.lstm3(input_tensor, (h2, c2))
        output, hupdated = self.lstm4(input_tensor, (h3, c3))
        output, hupdated = self.lstm5(input_tensor, (h4, c4))
        output, hupdated = self.lstm6(input_tensor, (h5, c5))
...
n_hidden1 = hidden_features_layer1
n_hidden2 = hidden_features_layer2
n_hidden3 = hidden_features_layer3
n_hidden4 = hidden_features_layer4
n_hidden5 = hidden_features_layer5
n_hidden6 = hidden_features_layer6
N_aminoAcids = len(aminoAcidLetters)

network = LSTM(N_nucleotides, n_hidden1, n_hidden2, n_hidden3, n_hidden4, n_hidden5, n_hidden6, num_layers, N_aminoAcids)
...
class LSTM(nn.Module):
    def __init__(self, 
        ...
        self.lstm1 = nn.LSTM(input_features, hidden_features_layer1, num_layers, batch_first=True)
        self.lstm2 = nn.LSTM(input_features, hidden_features_layer1, hidden_features_layer2, batch_first=True)
        self.lstm3 = nn.LSTM(input_features, hidden_features_layer2, hidden_features_layer3, batch_first=True)
        self.lstm4 = nn.LSTM(input_features, hidden_features_layer3, hidden_features_layer4, batch_first=True)

        self.fc = nn.Linear(hidden_features_layer4, num_of_classes, bias=False)

    def forward(self, input_tensor):
        h0 = torch.empty(self.num_layers, input_tensor.size(0), self.hidden_features_layer1)
        h0 = nn.init.orthogonal_(h0, gain=1).to(device)

        output, _ = self.lstm1(input_tensor, h0)
...
#plt.show()
#save figure:
plt.savefig(r"/content/deep-learning-project-cracking-the-genetic-code/FIGURES/LSTM2hiddenlayers40ep256weights.svg")
plt.savefig(r"/content/deep-learning-project-cracking-the-genetic-code/FIGURES/LSTM2hiddenlayers40ep256weights.pdf")
...
#-------------------------------------------------------------------------------------
filePATH = r"/content/deep-learning-project-cracking-the-genetic-code/FIGURES/"
ThisFileName = "LSTM2layers256_40epFreqTRue.txt"
scoresfile = open(filePATH+ThisFileName, 'w')
headerline = 'LSTM 2 layers hidden size 256 adjusting for weights.\n'
