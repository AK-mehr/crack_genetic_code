from tokenizerOHE_MLP8Layers import letter_to_index, letter_to_tensor, sequence_to_tensor, loadData, codon_to_tensor, \
    tensor_to_codon, index_to_tensor, updateANDsave_heatmap
from tokenizerOHE_MLP8Layers import nucleotideLetters, aminoAcidLetters, nucleotideLetterList, \
...
hidden_features_layer3 = 256
hidden_features_layer4 = 512
hidden_features_layer5 = 768
hidden_features_layer6 = 1024
hidden_features_layer7 = 1813
hidden_features_layer8 = 2858
learning_rate = 0.01
'''
                                                                num_epochs = 40
'''
num_epochs = 2 # number of iterations in the training loop
...
class MLP(nn.Sequential): # Multilayer Perceptron Network
    def __init__(self, input_features, 
                ...
                hidden_features_layer7,
                hidden_features_layer8):
        super().__init__(
            nn.Flatten(),
            ...
            nn.ReLU(), 
            nn.Linear(hidden_features_layer2, hidden_features_layer3, bias=False),
            nn.ReLU(), 
            nn.Linear(hidden_features_layer3, hidden_features_layer4, bias=False),
            nn.ReLU(),
            nn.Linear(hidden_features_layer4, hidden_features_layer5, bias=False),
            nn.ReLU(),
            nn.Linear(hidden_features_layer5, hidden_features_layer6, bias=False),
            nn.ReLU(), 
            nn.Linear(hidden_features_layer6, hidden_features_layer7, bias=False),
            nn.ReLU(), 
            nn.Linear(hidden_features_layer7, hidden_features_layer8, bias=False),
            ...
            nn.Linear(hidden_features_layer8, output_features, bias=False),
...
plt.savefig(r"/content/deep-learning-project-cracking-the-genetic-code/FIGURES/MLP_40iter64bits1024weights8layers.svg")
plt.savefig(r"/content/deep-learning-project-cracking-the-genetic-code/FIGURES/MLP_40iter64bits1024weights8layers.pdf")

# This produce a file saving the losses and train + test accuracies for each iteration
#-------------------------------------------------------------------------------------
# filePATH = r"/content/deep-learning-project-cracking-the-genetic-code/FIGURES/MLP64bits1024weights/"
...
ThisFileName = "MLP_40iter64bits1024weights8Layers.txt"
scoresfile = open(filePATH+ThisFileName, 'w')
headerline = 'MLP OHE 64 bits Hidden Layer 1024 with 40 epochs with adjustment for classes weights.\n'
scoresfile.write(headerline) # write header line just once
headerline = 'epoch' + '\t' + 'train_loss' + '\t' + 'test_loss' + '\t' + 'train_accuracy' + '\t' + 'test_accuracy' + '\n'
